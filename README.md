...
**第一种创建方式：使用命令行手动创建**
	...
	mvn archetype:generate -DgroupId=com.cheung.seckill -DartifactId=seckill -Dpackage=com.cheung.seckill -Dversion=1.0-SNAPSHOT -DarchetypeArtifactId=maven-archetype-webapp
...
* 点击`Next`，填写`groupId=com.cheung.seckill`，`DartifactId=seckill`，`package=com.cheung.seckill`（根据实际情况填写），然后`Finish`
...
>在`src/main/java`下建立`com.cheung.dao`包，在包下建立`SeckillDao`接口
...
>在`com.cheung.dao`包下建立`SuccessKilledDao`接口
...
我们也可以按照原本的习惯，在`src/main/java`下建立`com.cheung.mapper`包，将这些SQL映射存放到这里。由于Maven默认不会编译`src/main/java`下除源码以外的文件，所以需要在pom.xml中进行额外的配置。
...
>在`src/main/java`目录下的`com.cheung.mapper`包里创建SeckillDao.xml

	<!DOCTYPE mapper
	        PUBLIC "-//mybatis.org//DTD Mapper 3.0//EN"
	        "http://mybatis.org/dtd/mybatis-3-mapper.dtd">
	<!-- namespace:指定为哪个接口提供配置 --> 
	<mapper namespace="com.cheung.dao.SeckillDao">
...
>在`src/main/java`目录下的`com.cheung.mapper`包里创建SuccessKilledDao.xml
	
	<!DOCTYPE mapper
	        PUBLIC "-//mybatis.org//DTD Mapper 3.0//EN"
	        "http://mybatis.org/dtd/mybatis-3-mapper.dtd">
	<mapper namespace="com.cheung.dao.SuccessKilledDao">
...
>在`resources/spring`目录下创建Spring关于DAO层的配置文件spring-dao.xml
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
	    ...
	    <bean id="sqlSessionFactory" class="org.mybatis.spring.SqlSessionFactoryBean">
	        ...
	        <property name="typeAliasesPackage" value="com.cheung.entity"/>
	    ...
	    <bean class="org.mybatis.spring.mapper.MapperScannerConfigurer">
	        ...
	        <!-- 给出需要扫描的Dao接口-->
	        <property name="basePackage" value="com.cheung.dao"/>
	    </bean>
	
	    <!--redisDao-->
	   <!--  <bean id="redisDao" class="com.cheung.dao.cache.RedisDao">
...
>`SeckillDaoTest.java`
	...
	import com.cheung.entity.Seckill;
...
>`SuccessKilledDaoTest.java`
	...
	import com.cheung.entity.SuccessKilled;
